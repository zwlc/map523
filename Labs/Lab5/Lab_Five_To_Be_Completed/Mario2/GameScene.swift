...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createBird() {
        ...
        */
        ...
        var birdFly : [SKTexture] = []
        for number in 1...7 {
            birdFly.append(SKTexture(imageNamed: "bird-\(number).png"))
        }
        bird.run(SKAction.repeatForever(SKAction.animate(with: birdFly, timePerFrame: 0.2)))
        ...
        bird.position = CGPoint(x: size.width / 2 + bird.size.width / 2, y: coinY)
